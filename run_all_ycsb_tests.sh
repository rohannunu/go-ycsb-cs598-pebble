...
OPCOUNT=1000000000   # huge so time, not count, is the limiter
FIELDCOUNT=10
FIELDLEN=200
MAXEXEC=80           # seconds per run
...
run_experiment() {
  ...
  # ---------------------------
  # Examples:
  #   WorkloadVsDistribution_wlA_lru_zipfian_t32.out
  #   MultithreadScaling_wlA_detox_zipfian_t128.out
  local outfile="${group}_wl${wl}_${cache}_${dist}_t${threads}.out"
  ...
  cmd="bin/go-ycsb run ${db} \
    ...
    -p maxexecutiontime=${MAXEXEC}"

  # Hotspot extra params from your table (0.2, 0.8)
